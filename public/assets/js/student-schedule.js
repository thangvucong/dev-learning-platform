...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function formatRange(calendarApi) {
    ...
    var end = new Date(view.activeEnd.getTime() - 24 * 60 * 60 * 1000);
    ...
    if (dateRangeEl) dateRangeEl.textContent = "".concat(format(start), " - ").concat(format(end));
  }
  ...
  function updateDetail(session) {
    ...
    if (statusEl) {
      ...
      statusEl.className = "inline-flex items-center px-2 py-1 rounded-full border text-[10px] uppercase font-semibold ".concat(statusClassMap[status] || statusClassMap.upcoming);
