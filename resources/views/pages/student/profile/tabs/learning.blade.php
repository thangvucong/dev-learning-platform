<div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
    <div class="xl:col-span-2 space-y-4">
        <div class="rounded-2xl border border-slate-700 bg-slate-900/40 p-5">
            <h3 class="text-lg font-semibold text-white mb-4">Tiến độ theo mốc thời gian</h3>
            @include('components.student.profile.learning-timeline', ['timeline' => $learning['timeline']])
        </div>

        <div class="rounded-2xl border border-slate-700 bg-slate-900/40 p-5">
            <h3 class="text-lg font-semibold text-white mb-3">Module hoàn thành gần đây</h3>
            <div class="space-y-2">
    ...
    <div class="xl:col-span-1 space-y-4">
            ...
            <div class="space-y-2">
                @forelse ($learning['courses'] as $courseItem)
                    <div class="rounded-lg border border-slate-700 bg-slate-950/40 p-3">
                        ...
                        <p class="text-sm text-slate-400 mt-1">Giảng viên {{ $courseItem['mentor'] }}</p>
        ...
        <div class="rounded-2xl border border-slate-700 bg-slate-900/40 p-5">
            <h3 class="text-lg font-semibold text-white mb-3">Lớp học sắp tới</h3>
            <div class="space-y-2">
                @forelse ($learning['upcoming_classes'] as $classItem)
                    <div class="rounded-lg border border-slate-700 bg-slate-950/40 p-3">
                        <p class="text-sm text-slate-100">{{ $classItem['name'] }}</p>
                        <p class="text-sm text-slate-400 mt-1">{{ $classItem['start_at'] }}</p>
                    </div>
                @empty
