...
<div class="rounded-2xl border border-slate-700 bg-[#111827] overflow-hidden hover:border-slate-500 transition-colors">
    ...
    <div class="p-4">
        ...
        @if ($attendanceRate !== null)
            <p class="mt-1 text-[11px] text-slate-400">Attendance: {{ $attendanceRate }}%</p>
        @endif

        <div class="mt-4 flex items-center gap-2">
            <button type="button"
                class="inline-flex items-center justify-center h-9 w-9 rounded-lg border border-emerald-500/30 text-emerald-300 hover:bg-emerald-500/10 transition-colors"
                title="Vào học" aria-label="Vào học">
                <i class="fa-solid fa-right-to-bracket text-xs"></i>
            </button>
            <a href="{{ $detailUrl }}"
                class="inline-flex items-center justify-center h-9 w-9 rounded-lg border border-slate-600 text-slate-300 hover:bg-slate-700 transition-colors"
